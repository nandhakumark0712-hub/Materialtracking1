...
const Chat = () => {
   ...
   return (
      <div className="h-[calc(100vh-140px)] flex gap-8 animate-in fade-in duration-500">
         {/* Sidebar */}
         <div className="w-96 bg-white rounded-[3.5rem] shadow-sm border border-slate-100 flex flex-col overflow-hidden italic">
         ...
         {/* Chat Area */}
         <div className="flex-1 bg-white rounded-[3.5rem] shadow-sm border border-slate-100 flex flex-col overflow-hidden relative italic">
            <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/10">
               <div className="flex items-center space-x-4">
                  <div className={`w-14 h-14 rounded-2xl text-white flex items-center justify-center font-black italic shadow-lg ${activeChat.type === 'group' ? 'bg-primary-500 shadow-primary-500/20' : 'bg-slate-900 shadow-slate-900/20'}`}>
                     {activeChat.type === 'group' ? <Megaphone size={24} /> : activeChat.data.name.charAt(0)}
                  </div>
                  <div>
                     <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight">{activeChat.type === 'group' ? 'Announcement Hub' : activeChat.data.name}</h3>
                     <p className={`text-[10px] font-black uppercase tracking-widest flex items-center ${activeChat.type === 'group' ? 'text-primary-500' : 'text-slate-400'}`}>
                        {activeChat.type === 'group' ? 'Broadcasting to all sectors' : activeChat.data.role}
                     </p>
                  </div>
               </div>
               <div className="flex items-center space-x-4">
                  <button className="p-3 text-slate-300 hover:text-primary-500 transition-all"><Phone size={20} /></button>
                  <button className="p-3 text-slate-300 hover:text-primary-500 transition-all"><Video size={20} /></button>
                  <button className="p-3 text-slate-300 hover:text-primary-500 transition-all"><MoreVertical size={20} /></button>
               </div>
            </div>
               ...
               {activeChat.type === 'private' ? (
                  messages.map((m, idx) => (
                     <div key={idx} className={`flex ${String(m.sender) === String(currentUser.id) ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-md p-6 rounded-[2.5rem] shadow-sm ${String(m.sender) === String(currentUser.id) ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-white border border-slate-100 text-slate-700 rounded-tl-none'}`}>
                           <p className="text-sm font-medium">{m.content}</p>
                           <p className={`text-[9px] font-black uppercase mt-2 ${String(m.sender) === String(currentUser.id) ? 'text-white/40 text-right' : 'text-slate-400'}`}>
                  ...
                  announcements.map((a, idx) => (
                     <div key={idx} className="flex justify-start">
                        <div className="max-w-xl w-full p-8 bg-white border border-slate-100 rounded-[3rem] rounded-tl-none shadow-sm relative overflow-hidden">
                           <div className="absolute top-0 right-0 p-4 opacity-5"><Megaphone size={48} /></div>
                           <div className="flex items-center space-x-3 mb-4">
                              <span className="px-3 py-1 bg-rose-50 text-rose-600 text-[9px] font-black uppercase tracking-widest rounded-full">Protocol Update</span>
                              <span className="text-[10px] font-bold text-slate-300 uppercase">{new Date(a.createdAt).toLocaleDateString()}</span>
                           </div>
                           ...
                           <div className="mt-6 pt-6 border-t border-slate-50 flex items-center justify-between">
                              <div className="flex items-center space-x-2">
                                 ...
                                 <p className="text-[10px] font-black uppercase text-slate-900">{a.author?.name}</p>
                              </div>
                              <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">{a.author?.role}</span>
                           </div>
                        </div>
                     </div>
                  ))
            ...
            <div className="p-8 bg-white border-t border-slate-50">
               {activeChat.type === 'group' && currentUser.role !== 'Admin' ? (
                  <div className="bg-slate-50 p-6 rounded-[2.5rem] flex items-center justify-center border border-slate-100 space-x-3">
                     <ShieldAlert size={18} className="text-slate-400" />
                     <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Restricted Channel: Only high-level clearance can broadcast.</p>
                  </div>
               ) : (
                  <form onSubmit={handleSendMessage} className="flex items-center space-x-4 bg-slate-50 p-2 rounded-[2.5rem] border border-slate-100">
                     <button type="button" className="p-4 text-slate-400 hover:text-primary-500 transition-all"><Smile size={20} /></button>
                     <button type="button" className="p-4 text-slate-400 hover:text-primary-500 transition-all"><Paperclip size={20} /></button>
                     <input 
                        className="flex-1 bg-transparent border-none focus:ring-0 font-bold text-sm text-slate-700" 
                        placeholder={activeChat.type === 'group' ? "Broadcast announcement..." : "Transmit message..."}
                        value={message}
                        onChange={e => setMessage(e.target.value)}
                        disabled={sending}
                     />
                     <button type="submit" disabled={sending} className={`p-5 text-white rounded-[2rem] shadow-lg transition-all hover:scale-105 ${activeChat.type === 'group' ? 'bg-primary-500 shadow-primary-500/20' : 'bg-slate-900 shadow-slate-900/20'}`}>
