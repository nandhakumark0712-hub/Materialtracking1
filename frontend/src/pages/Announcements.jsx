   ...
   return (
      <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in duration-500 pb-20">
         <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
               <h1 className="text-5xl font-black italic tracking-tighter uppercase leading-none mb-4 text-slate-900">Announcement Hub</h1>
               <p className="text-slate-500 font-bold uppercase text-xs tracking-[0.3em] italic flex items-center">
                  <Info size={16} className="mr-3 text-primary-500" /> Organizational Directives & Protocol Updates
               </p>
            </div>
            <div className="bg-white px-8 py-4 rounded-[2rem] border border-slate-100 shadow-sm flex items-center space-x-4 italic">
               <div className="w-10 h-10 bg-primary-50 text-primary-500 rounded-2xl flex items-center justify-center">
                  <Megaphone size={20} />
               </div>
               <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Broadcasts</p>
                  <p className="text-lg font-black text-slate-900 leading-none mt-1">{announcements.length}</p>
               </div>
            </div>
         </div>
         ...
         {announcements.length === 0 ? (
            ...
            <div className="grid gap-8">
               {announcements.map((a) => (
                  <div key={a._id} className="group relative bg-white p-10 rounded-[4rem] border border-slate-100 shadow-sm hover:shadow-xl hover:border-primary-500/20 transition-all duration-500 overflow-hidden">
                     ...
                     <div className="flex flex-col md:flex-row gap-8 relative z-10">
                        ...
                        <div className="flex-1 space-y-6">
                           <div>
                              <h3 className="text-2xl font-black text-slate-900 uppercase italic tracking-tight leading-tight group-hover:text-primary-500 transition-colors">{a.title}</h3>
                              <div className="mt-6 p-8 bg-slate-50 rounded-[3rem] rounded-tl-none border border-slate-100 italic relative">
                                 <div className="absolute -left-2 top-0 w-4 h-4 bg-slate-50 rotate-45 border-l border-t border-slate-100"></div>
         ...
         <div className="bg-slate-900 rounded-[3.5rem] p-12 text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="flex items-center space-x-8 relative z-10">
               <div className="w-20 h-20 bg-primary-500 rounded-3xl flex items-center justify-center shadow-xl shadow-primary-500/20">
                  <ShieldAlert size={40} />
               </div>
               <div>
                  <h4 className="text-xl font-black italic uppercase tracking-tight mb-2">Protocol Enforcement</h4>
                  <p className="text-white/40 text-sm font-medium italic leading-relaxed max-w-2xl">
